...
if (!s.livingDexShiny) s.livingDexShiny = {};var state = load();
function load() {
var s = null;
try {
var raw = localStorage.getItem(STORE_KEY);
...
// less padding baked into its sprite than a mid-chain stage-1 form. This
// walks the chips in a given gen container, looks up each species' chain
// info via fetchEvoChainInfo (PokeAPI, cached per slug - see above), and
// flags anything except a 2-stage-line final with .dex-chip-stage-boost
// so CSS can zoom it in further. Only called on a gen actually being
// opened/expanded (not on every render) since the whole grid holds 1000+
// species and most of it stays collapsed/off-screen - see call sites in
// expandDexCard, expandKalosTile, and renderKalosMobileDex/renderLivingDex
// for the already-open case. Chips are marked with a dataset flag once
// checked so re-expanding a gen (or a data-only re-render of an
// already-open one) never re-issues the same lookup.
...
if (info && !(info.isFinal && info.stage === 1)) {
chip.classList.add('dex-chip-stage-boost');
}
});
});
}
